...
def fetch_directory_snapshots(session, target_file):

    base_patterns = [
        "http://www.informatics.jax.org/downloads/reports/",
        "https://www.informatics.jax.org/downloads/reports/",
        "http://informatics.jax.org/downloads/reports/",
        "https://informatics.jax.org/downloads/reports/",
    ]
        ...
        try:
            ...
            for row in data[1:]:
                ...
                try:
                    ...
                    if target_file.lower() not in original.lower():
                        continue
            ...
            print(f"Directory CDX failed: {e}")
    ...
    for variant_tpl in URL_VARIANTS:
        ...
        for status_filter in (True, False):
            try:
                ...
                # Directory-level searches
                dir_rows = fetch_directory_snapshots(session, target_file)

                # Merge
                all_rows = rows + dir_rows

                # Deduplicate by timestamp + original URL
                seen = set()
                deduped_rows = []

                for row in all_rows:
                    key = (row[0], row[2])  # timestamp, original URL
