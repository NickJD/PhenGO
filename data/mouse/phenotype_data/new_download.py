...
def fetch_snapshots_cdx(session, target_file, status_filter=True):
    ...
    status_filter=True  → only statuscode:200 (fast, misses some)
    status_filter=False → no status filter (catches redirected captures)
    """
    base_url = f"http://www.informatics.jax.org/downloads/reports/{target_file}"
    ...
    params = {
        ...
        "matchType": "exact",
        "limit":     "5000",   # pull more candidates upfront
    }
    if status_filter:
        params["filter"] = "statuscode:200"

    print(f"  CDX query (status_filter={status_filter}): {base_url}")
