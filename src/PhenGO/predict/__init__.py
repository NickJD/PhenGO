...
def __getattr__(name):
    ...
    # Lazy-import the owning submodule on first access
    from . import (data, model, evaluate, importance, visualise, compare, utils)
    _map = {
        "load_arff_data": data, "prepare_data": data,
        "create_model": model, "create_model_sparse_optimised": model,
        ...
        "plot_roc_curve": visualise, "plot_training_history": visualise,
        "plot_feature_importance_with_errors": visualise,
        "compare_datasets": compare,
        "predict_unlabeled_genes": utils, "cross_validate_model": utils,
    }
    return getattr(_map[name], name)
